...
import {
  Coins, LogOut, CreditCard, Zap, Calendar, RefreshCw, Crown,
} from "lucide-react"
...
export default function ProfileClient() {
  ...
  useEffect(() => {
    ...
    if (params.get("subscription") === "success") {
      setPurchaseSuccess("Subscription activated!")
      window.history.replaceState({}, "", "/profile")
    }
  }, [])
          ...
          {(creditInfo?.credits ?? 0) === 0 && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6">
              <p className="text-red-600">
                ❌ {t("credits_empty")}{" "}
                <a href="/pricing" className="underline font-semibold">{t("purchase_credits")}</a>
              </p>
            </div>
          )}
          {purchaseSuccess && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6">
              <p className="text-green-600">✅ Purchase successful! {purchaseSuccess} credits added.</p>
            </div>
          )}
          {(creditInfo?.credits ?? 0) > 0 && (creditInfo?.credits ?? 0) < 5 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
              <p className="text-yellow-600">⚠️ {t("credits_low").replace("{credits}", String(creditInfo?.credits))}</p>
            </div>
          )}
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                            record.status === "failed" ? "bg-red-100" : "bg-green-100"
                          }`}>
                            <span className={record.status === "failed" ? "text-red-500 text-lg" : "text-green-500 text-lg"}>
